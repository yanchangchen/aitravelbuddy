...
export const useTripStore = create((set) => ({
  ...
  setConciergeGreeting: (msg) => set({ conciergeMessages: [{ role: 'ai', content: msg }] }),
  startPlanning: (customGreeting) => set((state) => ({ 
    planStatus: 'planning', 
    planResult: null, 
    partialResult: {}, 
    agentProgress: {}, 
    ...
    selectedLocation: null, 
    autoRunPlan: false,
    conciergeMessages: customGreeting ? [{ role: 'ai', content: customGreeting }] : (state.conciergeMessages && state.conciergeMessages.length > 1 ? state.conciergeMessages : [{ 
      role: 'ai', 
  ...
  updateAgentProgress: (node, status, progress, data) => set((state) => {
    const nodeOrder = ['orchestrator_agent', 'itinerary_agent', 'food_retail_agent', 'hospitality_agent', 'purchasing_agent', 'quality_agent'];
